...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, 
	 ...
	 * `dst' surface.
	 */
	//Log("w:%x,src->pitch:%x,dst->pitch:%x",w,src->pitch,dst->pitch);
	for(int temp = h-1; temp >= 0;temp--)
	{
		uint8_t* src_addr = (uint8_t*)src->pixels + (sy + temp) * src->pitch + sx;
		uint8_t* dst_addr = (uint8_t*)dst->pixels + (dy + temp) * dst->pitch + dx;
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
	...
	for(int temp = h-1; temp >= 0; temp--)
	{
		for(int i = 0; i < w; i++)
		{
			uint8_t* addr = (uint8_t*)dst->pixels + ( y + temp ) * dst->pitch + x + i;
			*addr = color;
		}
	}
	//assert(0);
